...
simpleLandLeft n (left, right) = (left + n, right)
  
simpleLandRight :: Birds -> Pole -> Pole
simpleLandRight n (left, right) = (left, right + n)
...
simpleLandTwoLeftTwoRight pole = pole -: simpleLandLeft 2 -: simpleLandRight 2
...
simpleLandingChain birds pole = pole -: simpleLandLeft 1 -: simpleLandRight birds -: simpleLandLeft (-1) -: simpleLandRight (-2)
...
Tip: Here you need to use two of the functions (or Methods) defined in Monad class.
     You can use :info Monad in ghci or Hoogle Monad for more info.
-}
landTwoLeftTwoRight :: Pole -> Maybe Pole
landTwoLeftTwoRight pole = return pole >>= landLeft 2 >>= landRight 2
...
landingChain birds pole = return pole >>= landLeft 1 >>= landRight birds
...
getStart = do
  putStrLn "Start?"
  input <- getLine
  -- reads's signature can be thought as Read a => String -> [(a, String)]
  -- or [(parsed value, remaining String)]
